...
```{r init, echo=F, eval=F,include=F}
library(readr)
library(tidyverse)
library(janitor)
library(quanteda)
...
```{r read_data1, warning=F, cache=T, message=F, eval=F}
...
```{r unify_datasets, warning=F, cache=T, message=F, eval=F}
...
```{r filter_unique, warning=F, cache=T, message=F, eval=F}
...
```{r jac_dist, warning=F, cache=T, message=F, eval=F}
...
```{r jac_dist_exploration, echo=T, eval=F, warning=F, cache=T, message=F, eval=F}
#### Data exploration #### 
...
```{r jac_dist_exploration2, echo=T, eval=F, warning=F, cache=T, message=F}
...
```
## 5. Explore theorists
```{r explore_theorists, warning=F, cache=T, message=F, eval=F}
##### Fetch data #####
...
```{r statistics2, warning=F,echo=T, cache=T,eval=F}
...
```{r statistics4, warning=F,echo=T, cache=T, eval=F}
